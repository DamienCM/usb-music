...
class USBMusicSkill(CommonPlaySkill):
    ...
    def search_music_youtube(self,phrase):
        ...
        process = subprocess.run(["yt-dlp", "-x", "-f", "mp4", f"ytsearch1:{phrase[:-10]}","--geo-bypass", "--metadata-from-title", '"(?P<artist>.+?) - (?P<title>.+)"',"--xattrs","--embed-thumbnail","-o","/home/pi/.usb-music/temp/request"], stdout=subprocess.PIPE)
